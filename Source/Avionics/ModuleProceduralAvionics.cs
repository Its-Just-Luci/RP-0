...
//        private float GetMaximumControllableMass() => FloorToSliderIncrement(GetControllableMass(MaxAvionicsMass));
        ...
        private MethodInfo GetSeekVolumeMethod()
        {
            if (_procPartPM != null)
            {
                ...
                MethodInfo mi = _procPartPM.GetType().GetMethod("SeekVolume", flags, null, new Type[] { typeof(float), typeof(int) }, null);
                mi ??= _procPartPM.GetType().GetMethod("SeekVolume", flags);    // Old PP only has method with 1 argument
                return mi;
            }
            return null;
        }
